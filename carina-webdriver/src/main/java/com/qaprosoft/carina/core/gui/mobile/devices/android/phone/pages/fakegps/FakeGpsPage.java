...
public class FakeGpsPage extends MobileAbstractPage {
    ...
    public void solveMockSettings() {
        if (openSettingsButton.isElementPresent(MINIMAL_TIMEOUT)) {
            ...
            ExtendedWebElement allowMockElement = allowMock;
            
            if (currentAndroidVersion.contains("7.")) {
            	allowMockElement = allowMock7;
            }
            MobileUtils.swipeToElementInsideContainer(devSettingsContainer, allowMockElement.getBy());
            LOGGER.info("Allow Mock config is present:" + allowMockElement.isElementPresent(SHORT_TIMEOUT));
            allowMockElement.clickIfPresent(MINIMAL_TIMEOUT);
